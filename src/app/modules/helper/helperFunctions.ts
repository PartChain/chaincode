...
import AssetValidator, {
  InvestigationClass,
  AddOrgToInvestigationClass
} from "../../models/ValidationClasses";
...
export const checkAccessPermission = async (
  ...
  if (!targetOrgDataDetails["ACL"].hasOwnProperty(RequestID)) {
    ...
    return await returnFunction(
      `RequestID ${RequestID} doesnot exist with ${targetOrgMSP}`,
  ...
  if (targetOrgDataDetails["ACL"][RequestID]["status"] != "ACTIVE") {
    ...
    return await returnFunction(
      `RequestID ${RequestID} is not in ACTIVE status`,
      ResponseStatus.PERMISSION_DENIED
    );
  }
...
export const validateAssetParameters = async (payload: any) => {
  ...
  assetPayload.productionCountryCodeManufacturer =
    payload.productionCountryCodeManufacturer;
...
export const validateCreateInvestigation = async (payload: any) => {
  ...
  // Asset details for validation
  investigationPayload.message = payload.message;
...
export const validateAsset = async (
  componentSharedHash: string,
  asset: AssetHashVerificationModel
) => {
...
export const decryptArray = async (
  keyString: string,
  ivString: string,
  inputArray: string[]
) => {
...
export const encryptArray = async (
  keyString: string,
  ivString: string,
  inputArray: string[]
) => {
 ...
 * @param input
 */
export const decryptParticipatingOrgs = async (
  keyString: string,
  key2String: string,
  ivString: string,
  input: any
  ...
  for (encryptedkey in input) {
    if (input.hasOwnProperty(encryptedkey)) {
      ...
      input[encryptedkey]["mspID"] = await decryptData(
        keyString,
        ivString,
        input[encryptedkey]["mspID"]
      );
      if (input[encryptedkey]["componentsSerialNumbers"].length > 0) {
        input[encryptedkey]["componentsSerialNumbers"] = await decryptArray(
          key2String,
          ivString,
          input[encryptedkey]["componentsSerialNumbers"]
        );
      }
...
export const returnFunction = async (payload: string, status: number) => {
  Logger.info(
    `Payload in the return function response =${payload}, status = ${status}`
  );
  return {
    data: payload,
    status: status
